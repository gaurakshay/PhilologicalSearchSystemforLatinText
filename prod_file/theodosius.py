#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Program to download theodosius text."""
...
def update_book_details(base_url, html):
    ...
    # Find child links with theodosius in them.
    child_links = html.find_all(href=re.compile('theodosius'))
    booknames = []
    ...
    theodosius_json.update({u'books' : booknames})
    #print (json.dumps(theodosius_json, indent=4,
    #                  separators=(',',': '),
    #                  ensure_ascii=False)).encode('utf-8')


def test_theodosius_download():
    """Test correctness of download."""
    print
    base_url = 'http://www.thelatinlibrary.com/'
    author_url = 'theodosius.html'
    ...
    theodosius_json.update({u'author_name' : author_name})
    theodosius_json.update({u'title' : re.sub(r'.html', '', author_url)})
    theodosius_json.update({u'dates' : u'null'})
    theodosius_json.update({u'language' : u'latin'})
    update_book_details(base_url, html_data)
    with codecs.open('theodosius.txt', 'w', encoding='utf-8') as f:
        f.write(json.dumps(theodosius_json, indent=4,
                           separators=(',', ': '),
                           ensure_ascii=False))
