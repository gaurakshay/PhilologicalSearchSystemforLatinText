#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Program to download plautus text."""
...
def update_book_details(base_url, html):
    ...
    # Find child links with plautus in them.
    child_links = html.find_all(href=re.compile('plautus'))
    booknames = []
    ...
    plautus_json.update({u'books' : booknames})
    #print (json.dumps(plautus_json, indent=4,
    #                  separators=(',',': '),
    #                  ensure_ascii=False)).encode('utf-8')


def test_plautus_download():
    """Test correctness of download."""
    print
    base_url = 'http://www.thelatinlibrary.com/'
    author_url = 'plautus.html'
    html_data = BeautifulSoup(get_html_data(base_url + 
                                            author_url),
                              'html.parser')
    author_tag = html_data.find('h1')
    author_name = (author_tag.get_text(" ", strip=True))
    date_ = html_data.find(class_='date').get_text(' ', strip=True)[6:-1]
    plautus_json.update({u'author_name' : author_name})
    plautus_json.update({u'title' : re.sub(r'.html', '', author_url)})
    plautus_json.update({u'dates' : date_})
    plautus_json.update({u'language' : u'latin'})
    update_book_details(base_url, html_data)
    with codecs.open('plautus.txt', 'w', encoding='utf-8') as f:
        f.write(json.dumps(plautus_json, indent=4,
                           separators=(',', ': '),
                           ensure_ascii=False))
