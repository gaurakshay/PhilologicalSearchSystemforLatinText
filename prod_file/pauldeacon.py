#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Program to download pauldeacon text."""
...
def update_book_details(base_url, html):
    ...
    # Find child links with pauldeacon in them.
    child_links = html.find_all(href=re.compile('pauldeacon'))
    booknames = []
    ...
    pauldeacon_json.update({u'books' : booknames})
    #print (json.dumps(pauldeacon_json, indent=4,
    #                  separators=(',',': '),
    #                  ensure_ascii=False)).encode('utf-8')


def test_pauldeacon_download():
    """Test correctness of download."""
    print
    base_url = 'http://www.thelatinlibrary.com/'
    author_url = 'pauldeacon.html'
    ...
    pauldeacon_json.update({u'author_name' : author_name})
    pauldeacon_json.update({u'title' : re.sub(r'.html', '', author_url)})
    pauldeacon_json.update({u'dates' : date_})
    pauldeacon_json.update({u'language' : u'latin'})
    update_book_details(base_url, html_data)
    with codecs.open('pauldeacon.txt', 'w', encoding='utf-8') as f:
        f.write(json.dumps(pauldeacon_json, indent=4,
                           separators=(',', ': '),
                           ensure_ascii=False))
