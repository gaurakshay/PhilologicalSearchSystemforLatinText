#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Program to download William Tyre text."""
...
def update_book_details(base_url, html):
    ...
    # Find child links with williamtyre in them.
    child_links = html.find_all(href=re.compile('williamtyre'))
    booknames = []
    ...
    williamtyre_json.update({u'books' : booknames})
    print (json.dumps(williamtyre_json, indent=4,
                      separators=(',',': '),
                      ensure_ascii=False)).encode('utf-8')


def test_williamtyre_download():
    """Test correctness of download."""
    print
    base_url = 'http://www.thelatinlibrary.com/'
    author_url = 'williamtyre.html'
    ...
    williamtyre_json.update({u'author_name' : author_name})
    williamtyre_json.update({u'title' : re.sub(r'.html', '', author_url)})
    williamtyre_json.update({u'dates' : date_})
    williamtyre_json.update({u'language' : u'latin'})
    update_book_details(base_url, html_data)
    with codecs.open('williamtyre.txt', 'w', encoding='utf-8') as f:
        f.write(json.dumps(williamtyre_json, indent=4,
                           separators=(',', ': '),
                           ensure_ascii=False))
